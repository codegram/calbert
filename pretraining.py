...
MODEL_CLASSES = {"albert": (AlbertConfig, AlbertForMaskedLM)}


class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", block_size=64):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for line in input_text:
                n += 1
                self.examples.append(tokenizer.encode(line.strip(), seq_len=block_size))
...
def mask_tokens(
    inputs: torch.Tensor, tokenizer: CalbertTokenizer, args
...
def main():
    ...
    if args.model_type in ["albert"] and not args.mlm:
        raise ValueError(
    ...
    config = AlbertConfig(**opts)

    tokenizer = CalbertTokenizer(
        "tokenizer/ca.bpe.30000-vocab.json", "tokenizer/ca.bpe.30000-merges.txt"
    )
